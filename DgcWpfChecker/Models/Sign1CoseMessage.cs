...
namespace DGC
{
    public class Sign1CoseMessage
    {
        ...
        public string ContextSignature1 = "Signature1";
        ...
        { 
            ...
            var protMapJson=protectedMap.ToJSONString();
            ...
            coseMsg.ProtectedMapBytes = protectedBytes;
        ...
        public bool VerifySignature(AsymmetricKeyParameter pubKey, string pContextSignature1,
            CBORObject pProtectedMap, byte[] pContent, byte[] pSignature, byte[] pProtectedMapBytes)
        {
            ISigner signer;
            var signature = pSignature;
            if (pubKey is Org.BouncyCastle.Crypto.Parameters.ECKeyParameters)
            {
                signer = SignerUtilities.GetSigner("SHA-256withECDSA");
                signature = ConvertConcatToDer(pSignature);
            }
            ...
            //var protMap = pProtectedMap.EncodeToBytes();

            var cborArray = CBORObject.NewArray();
            cborArray.Add(pContextSignature1);
            cborArray.Add(pProtectedMapBytes);
            cborArray.Add(new byte[0]); // no externaldata
            cborArray.Add(pContent);
