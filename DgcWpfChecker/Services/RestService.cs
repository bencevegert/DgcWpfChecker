...
namespace DGCValidator.Services
{
    public class RestService : IRestService
    {
        ...
        public async Task<DSC_TL> RefreshTrustListAsync(string pUrl)
        {
        ...
        public async Task<string> RefreshTrustListAsyncAT(string pUrl)
        {
            string vResult = null;
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    var b = CBORObject.DecodeFromBytes(bytes);
                    
                    var c = b.Values.FirstOrDefault();

                    vResult = c.ToJSONString();
                   
                    //JObject vResponseJson = (JObject)JsonConvert.DeserializeObject(content);
                    
                    //var vTrustList = vResponseJson["trustList"]["trustListContent"].ToString();
                    //var vBytes = Base64UrlDecode(json);
                    
                    //var vCbor = CBORObject.DecodeFromBytes(vBytes);

                    //var all=vCbor.ToJSONString();

                    //vResult = vCbor.Values.FirstOrDefault().ToJSONString();
        ...
        public static byte[] ToDerSignature(byte[] jwsSig)
        {
